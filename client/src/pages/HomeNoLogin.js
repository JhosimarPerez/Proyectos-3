  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        // Separar eventos con prioridadImagen
        const prioritizedImages = data
          .filter((event) => event.prioridadImagen === 1)
          .map((event) => ({
            src: `http://localhost:3001${event.rutaImagen}`,
            link: `/eventDetails/${event.evento_id}`, // Navegación al evento
            alt: event.titulo,
          }));

        setSliderImages(prioritizedImages); // Guardar imágenes prioritarias en el slider
        setEvents(data); // Guardar todos los eventos
      } catch (error) {
